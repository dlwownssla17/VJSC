...
public class UserSchedule {
	...
	public void ensureDate(Date date) {
		if (!this.items.containsKey(date)) this.items.put(date, new ArrayList<>());
	}
	...
	public synchronized void addScheduleItem(int year, int month, int day,
			int recurringType, int[] recurringValue, int endType, String endValue,
			String title, String description, String typeString,
			String startTimeString, String progressTypeString, int duration) {
		...
		if (recurringType > 4) {
			...
			Date recurrenceStartDateTime = scheduleItemStartDateTimeFromStartTimeString(startDate,
					startTimeString);
			if (endType == 0) { // never
				recurrence = new ScheduleItemRecurrence(this.recurringIdCounter++, recurringType,
						recurringValue, recurrenceStartDateTime);
			} else if (endType == 1) { // after Y occurrences
				recurrence = new ScheduleItemRecurrence(this.recurringIdCounter++, recurringType,
						recurringValue, recurrenceStartDateTime, Integer.parseInt(endValue));
			} else if (endType == 2) { // on T
				try {
					Date endDateTime = DateFormat.getDate(endValue, Constant.DATE_FORMAT);
					...
					recurrence = new ScheduleItemRecurrence(this.recurringIdCounter++, recurringType,
							recurringValue, recurrenceStartDateTime, DateAndCalendar.calendarToDate(calendar));
	...
	private void addSingleScheduleItem(Date date, ScheduleItemRecurrence recurrence,
			String title, String description, String typeString,
			String startTimeString, String progressTypeString, int duration) {
		this.ensureDate(date);
		...
		ScheduleItem scheduleItem = new ScheduleItem(this.scheduleIdCounter++, title, description,
				type, this.associatedUser, startDateTime, progress, null); // TODO: notification params
	...
	public synchronized void editScheduleItem(int year, int month, int day,
	...
	public synchronized void removeScheduleItem(int year, int month, int day,
			long id, long recurringId) {
		Date date = CreateLookupDate.getInstance(year, month, day);
		this.ensureDate(date);
		...
		if (recurringId == -1) {
			...
			if (removeIndex >= 0) listOfScheduleItems.remove(removeIndex);
			return;
		}
	...
	public synchronized ProgressColor getProgressColor(Date date) {
		...
		if (Math.max(Math.max(greenCount, yellowCount), redCount) == 0) return ProgressColor.GREY;
		if (greenCount >= yellowCount && greenCount >= redCount) return ProgressColor.GREEN;
		if (yellowCount >= redCount) return ProgressColor.YELLOW;
